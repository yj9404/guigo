...
function delay( timeout ) {
  ...
  puppeteer.launch({
     ...
     , devtools : true     //밑에는 크롬 경로 입력해주세요.
        ...
        await delay(4000);
        await page.goto("https://job.sogang.ac.kr/alba/school/", { waitUntil: "networkidle0" });
        await delay(4000);
        ...
        for (var i = 1; i <= len; i++) {
            await delay(300);
            var re = await page.waitFor(`div#wrap > div#contentsWrap > div#contents > table.listTable.tdCenter.spaceB20 > tbody > tr:nth-child(${i}) > td:nth-child(3) > a `)
            var rere = await page.evaluate(re => re.textContent, re);
            var uu = await page.waitFor(`div#wrap > div#contentsWrap > div#contents > table.listTable.tdCenter.spaceB20 > tbody > tr:nth-child(${i}) > td:nth-child(3) > a `)
           ...
           rere=rere.trim();
            var de = await page.waitFor(`div#wrap > div#contentsWrap > div#contents > table.listTable.tdCenter.spaceB20 > tbody > tr:nth-child(${i}) > td:nth-child(1)`)
