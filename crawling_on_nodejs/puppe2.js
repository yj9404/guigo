...
function delay( timeout ) {
  ...
  puppeteer.launch({
     ...
     , devtools : true     //밑에는 크롬 경로 입력해주세요.
        ...
        await delay(4000);
        await page.goto("https://job.sogang.ac.kr/jobs/sogang/recommendation/default.aspx?firstorderby=1&isadregdate=1", { waitUntil: "networkidle0" });
        await delay(4000);
        ...
        for (var i = 1; i <= len; i++) {
            await delay(300);
            var re = await page.waitFor(`div#wrap > div#contentsWrap > div#contents > table.listTable.spaceB20.cr > tbody > tr:nth-child(${i}) > td:nth-child(4)`)
            var rere = await page.evaluate(re => re.textContent, re);
            var uu = await page.waitFor(`div#wrap > div#contentsWrap > div#contents > table.listTable.spaceB20.cr > tbody > tr:nth-child(${i}) > td:nth-child(4) > a`)
