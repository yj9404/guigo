...
var BoardId = 16;//취지팀-과외정보
var mysql = require('mysql');
const puppeteer = require("puppeteer");
var func=function() {
var db = mysql.createConnection({
    host: "gwajeda.cpycvchscjku.ap-northeast-2.rds.amazonaws.com",
    user: "mscs20190627",
    password: "tngkrtlqkf12!",
    database: "gwajeda_schema"
});
db.connect(() => {
...
function delay( timeout ) {
  ...
  puppeteer.launch({
     ...
     , devtools : true     //밑에는 크롬 경로 입력해주세요.
        ...
        await delay(4000);
        await page.goto("https://job.sogang.ac.kr/alba/lesson/ ", { waitUntil: "networkidle0" });
        await delay(4000);
        ...
        for (var i = 1; i <= len; i++) {
            await delay(300);
            var rere="희망과목 : ";
            var re = await page.waitFor(`div#wrap > div#contentsWrap > div#contents > table.listTable.spaceB20.tdCenter > tbody > tr:nth-child(${i}) > td:nth-child(5) > a `)
            var subject = await page.evaluate(re => re.textContent, re);
            var uu = await page.waitFor(`div#wrap > div#contentsWrap > div#contents > table.listTable.spaceB20.tdCenter > tbody > tr:nth-child(${i}) > td:nth-child(5) > a `)
            var posturl = await page.evaluate(uu => uu.href, uu);
            //console.log(posturl);
            subject.replace(/'/gi, '"');
            rere+=subject+" | 지역 : ";
            var re = await page.waitFor(`div#wrap > div#contentsWrap > div#contents > table.listTable.spaceB20.tdCenter > tbody > tr:nth-child(${i}) > td:nth-child(6)`)
            var region = await page.evaluate(re => re.textContent, re);
            region.replace(/'/gi, '"');
            rere+=region;
            rere=rere.trim();
            var de = await page.waitFor(`div#wrap > div#contentsWrap > div#contents > table.listTable.spaceB20.tdCenter > tbody > tr:nth-child(${i}) > td:nth-child(2) > a`)
